...
**Errors:** Return `error` as the last return value for any fallible operation. Wrap with context: `fmt.Errorf("doing X: %w", err)`. Never ignore errors. Use sentinel errors for matchable conditions; custom error types for structured domain errors. No panic for expected errors.

**Naming:** Exported = UpperCamelCase, unexported = lowerCamelCase. No vague or abbreviated names.
